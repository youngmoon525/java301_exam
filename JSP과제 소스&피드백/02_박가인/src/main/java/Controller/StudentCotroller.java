...
//*.aa
//MVC패턴 ? M이 데이터이고 V가 view즉 jsp이고 C가 controller
//자바에 있는 것을 controller = servlet에 불러와서 다시 
// <a href="test.aa"> </a>
...
public class StudentCotroller extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(req.getServletPath().equals("/list.st")) {	
			...
			//first_name,last_name 만 수정되게끔 처리
			//업데이트 쿼리를 실행하고 나서 0보다 큰 숫자가 리턴되는지를 체크해보기
			//rd = req.getRequestDispatcher("student/update.jsp");
			//String first_name, String last_name, String student_no, String user_id
			
			int result = dao.updateInfo(req.getParameter("first_name"),req.getParameter("last_name"),req.getParameter("student_no"), req.getParameter("user_id"));
			System.out.println(result);
			resp.sendRedirect("list.st"); 
			return; 
			
			//forward 할 필요가 없음 
			//System.out.println(dao.updateInfo(req.getParameter("first_name"),req.getParameter("last_name"),req.getParameter("student_no"), req.getParameter("user_id")));
